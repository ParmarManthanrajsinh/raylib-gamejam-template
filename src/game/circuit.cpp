...
static int EvaluateGate(GateType type, int a, int b) {
    switch (type) {
        case GateType::AND:  return (a == 1 && b == 1) ? 1 : 0;
        case GateType::OR:   return (a == 1 || b == 1) ? 1 : 0;
        case GateType::NOT:  return a == 1 ? 0 : 1;
        case GateType::XOR:  return (a != b) ? 1 : 0;
        case GateType::NAND: return (a == 1 && b == 1) ? 0 : 1;
        case GateType::NOR:  return (a == 1 || b == 1) ? 0 : 1;
        case GateType::XNOR: return (a == b) ? 1 : 0;
    }
    return 0;
}
...
int EvaluateCircuit(
    ...
    for (const auto& gate : gates) {
        gate_outputs[gate.id] = 0;
    }
    for (int i = 0; i < 4; i++) output_bits[i] = 0;

    // Build wire target lookup: "to_type:to_id:to_pin" -> source info
    struct SourceInfo { int type, id, pin; };
    std::unordered_map<int, SourceInfo> wire_targets;
    ...
    auto get_source_value = [&](int src_type, int src_id, int src_pin) -> int {
    ...
    int max_iters = (int)gates.size() + 10;
    for (int iter = 0; iter < max_iters; iter++) {
        ...
        for (int b = 0; b < 4; b++) {
            ...
            int new_val = 0;
            if (it != wire_targets.end()) {
                new_val = get_source_value(it->second.type, it->second.id, it->second.pin);
            }
    ...
    // Compute hex value
    return output_bits[0] + output_bits[1] * 2 + output_bits[2] * 4 + output_bits[3] * 8;
}
