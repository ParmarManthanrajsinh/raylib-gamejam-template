...
void DrawLevelComplete(float anim_time, float transition_time, const GameStats& stats, GameState game_state)
{
    ...
    if (confetti_init)
    {
        ...
        for (int i = 0; i < 100; i++)
        {
            ...
            if (confetti[i].y < SCREEN_HEIGHT + 20)
            {
                ...
                // Confetti glow
                DrawRectangleV({confetti[i].x - size/2, confetti[i].y - size/2}, {size*2, size*2}, ColorAlpha(confetti_colors[i], 0.3f));
            }
        }
    }
    ...
    DrawRectangleRoundedLines({panel_x, panel_y, current_w, current_h}, 0.1f, 8, ColorAlpha({0, 255, 255, 255}, 0.8f * scale));
    DrawRectangleRoundedLines({panel_x-2, panel_y-2, current_w+4, current_h+4}, 0.1f, 8, ColorAlpha({255, 0, 255, 255}, 0.4f * scale)); // double border
    ...
    bool is_outbound = (game_state == GameState::LEVEL_COMPLETE_TO_PLAY_TRANSITION || game_state == GameState::LEVEL_COMPLETE_TO_TITLE_TRANSITION);
    ...
    // Shadow
    DrawTextShadowed(font, title, static_cast<int>(tx), static_cast<int>(ty + 4), 32 * scale, ColorAlpha(BLACK, 0.8f));
    // Red split
    ...
    DrawLineEx({panel_x + 40*scale, div1_y}, {panel_x + panel_w*scale - 40*scale, div1_y}, 2.0f, ColorAlpha({255, 0, 255, 255}, 0.5f));
    ...
    if (is_outbound || (game_state == GameState::LEVEL_COMPLETE && state_time > 0.2f)) {
        DrawTextShadowed(font, "Time:", static_cast<int>(left_col_x), static_cast<int>(y_pos), 22 * scale, WHITE);
        std::snprintf(buf, sizeof(buf), "%.1fs", stats.time_taken);
        float sw = MeasureTextEx(font, buf, 22 * scale, 1).x;
        DrawTextShadowed(font, buf, static_cast<int>(right_col_x - sw), static_cast<int>(y_pos), 22 * scale, {0, 255, 255, 255}); // Cyan
    }
    y_pos += 40 * scale;

    if (is_outbound || (game_state == GameState::LEVEL_COMPLETE && state_time > 0.4f)) {
        DrawTextShadowed(font, "Gates Used:", static_cast<int>(left_col_x), static_cast<int>(y_pos), 22 * scale, WHITE);
        std::snprintf(buf, sizeof(buf), "%d", stats.gates_used);
        float sw = MeasureTextEx(font, buf, 22 * scale, 1).x;
        DrawTextShadowed(font, buf, static_cast<int>(right_col_x - sw), static_cast<int>(y_pos), 22 * scale, {255, 0, 255, 255}); // Magenta
    }
    y_pos += 40 * scale;

    if (is_outbound || (game_state == GameState::LEVEL_COMPLETE && state_time > 0.6f)) {
        DrawTextShadowed(font, "Wires Used:", static_cast<int>(left_col_x), static_cast<int>(y_pos), 22 * scale, WHITE);
        std::snprintf(buf, sizeof(buf), "%d", stats.wires_used);
        float sw = MeasureTextEx(font, buf, 22 * scale, 1).x;
        DrawTextShadowed(font, buf, static_cast<int>(right_col_x - sw), static_cast<int>(y_pos), 22 * scale, {255, 255, 0, 255}); // Yellow
    }
    y_pos += 50 * scale;

    // Divider Line 2
    float div2_y = panel_y + 265 * scale;
    DrawLineEx({panel_x + 40*scale, div2_y}, {panel_x + panel_w*scale - 40*scale, div2_y}, 2.0f, ColorAlpha({0, 255, 255, 255}, 0.5f));
    ...
    if (is_outbound || (game_state == GameState::LEVEL_COMPLETE && state_time > 1.0f)) {
        ...
        DrawTextShadowed(font, buf, static_cast<int>((SCREEN_WIDTH - sw)/2), static_cast<int>(y_pos), 26 * scale, ColorAlpha(WHITE, 0.8f + 0.2f * r_pulse));
        ...
        DrawTextShadowed(font, buf, static_cast<int>((SCREEN_WIDTH - rw)/2), static_cast<int>(y_pos), 20 * scale, rank_col);
    }
    ...
    if (is_outbound || (game_state == GameState::LEVEL_COMPLETE && state_time > 1.5f)) {
        ...
        auto draw_cyber_btn = [&](Rectangle r, bool hov, Color base_col, const char* txt) {
            ...
            Vector2 pts[6] = {
                {r.x + c_cut, r.y}, {r.x + r.width, r.y},
                {r.x + r.width, r.y + r.height - c_cut}, {r.x + r.width - c_cut, r.y + r.height},
                {r.x, r.y + r.height}, {r.x, r.y + c_cut}
            };
            ...
            // Tech detail lines
            if (hov) {
                DrawLineEx({r.x + 5*scale, r.y + r.height - 5*scale}, {r.x + 20*scale, r.y + r.height - 5*scale}, 2.0f*scale, WHITE);
            }

            float tw = MeasureTextEx(font, txt, 22 * scale, 2).x;
            DrawTextShadowed(font, txt, static_cast<int>(r.x + (r.width - tw)/2), static_cast<int>(r.y + 14 * scale), 22 * scale, hov ? WHITE : ColorAlpha(WHITE, 0.8f));
        };
