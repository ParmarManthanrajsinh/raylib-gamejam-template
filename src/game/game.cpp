...
void Game::Draw()
{
    ...
    bool cell_occupied = hovered_cell.IsValid() &&
    ...
    for (const auto& gate : gates)
    {
        ...
        Color oc = hov_out ? SKYBLUE :
            (out_val ? Color{ 255, 96, 128, 255 } : Color{ 85, 119, 187, 255 });
    ...
    if (selected_gate_index >= 0)
    {
        ...
        else
        {
            ...
            DrawTextShadowed(font, name, static_cast<int>(gp.x - ts.x/2), static_cast<int>(gp.y + gh*s/2 + 6), 8, ColorAlpha(WHITE, 0.5f));
        }
    }
